...
export default async function SimulationPage({ params }: Props) {
  ...
  if (simulation.availability === "live" && slug === "vertical-throw") return <SimulationAccessGate><SimulationLab simulation={simulation}/></SimulationAccessGate>;

  if (simulation.availability === "live") {
    return <SimulationAccessGate><div className="content-shell pb-24 pt-8"><Link className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-[#C7C5CC]/80 hover:text-white" href="/simulations"><ArrowLeft size={17}/> All simulations</Link><div className="mt-5"><FreeSimulationPlayground initialSlug={slug}/></div></div></SimulationAccessGate>;
  }
