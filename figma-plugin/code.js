(async function () {
  ...
  const F = {
    display: await font([{ family: "Hanken Grotesk", style: "SemiBold" }, { family: "Hanken Grotesk", style: "Regular" }, { family: "DM Sans", style: "Medium" }, { family: "Inter", style: "Regular" }]),
    displayBold: await font([{ family: "Hanken Grotesk", style: "Bold" }, { family: "Hanken Grotesk", style: "SemiBold" }, { family: "DM Sans", style: "Bold" }, { family: "Inter", style: "Bold" }]),
  ...
  function learningRail(root, height, active) {
    ...
    ["Home", "Visual notes", "Simulations"].forEach(function (label, i) {
      const y = 134 + i * 48;
    ...
    text(node, "Library", "Your library", 24, 306, { font: F.bold, size: 12, color: C.muted });
    text(node, "Bookmarks", "Bookmarks", 58, 349, { font: F.bold, size: 14, color: active === "Bookmarks" ? C.paper : C.muted });
    text(node, "Account", "Account", 24, 414, { font: F.bold, size: 12, color: C.muted });
    ["Profile", "Settings", "Help centre"].forEach(function (label, i) { text(node, "Account / " + label, label, 58, 455 + i * 48, { font: F.bold, size: 14, color: active === label ? C.paper : C.muted }); });
  }
  ...
  function legacyHomeFrame(page, index) {
    const root = frame(page, "Frame / " + String(index).padStart(2, "0") + " / Home / /", 0, 0, 1440, 4249, C.bg);
    ...
    [["\u2713", "Visual notes"], ["\u2713", "Interactive simulations"]].forEach(function (value, i) {
      const x = 56 + [0, 128][i]; text(hero, "Proof / Check / " + i, value[0], x, 552, { font: F.bold, size: 14, color: C.green });
    ...
    const faqs = frame(root, "Section / 06 / FAQs", 0, 3119, 1440, 686, C.surface, C.hair);
    text(faqs, "Heading", "Frequently asked\nquestions", 56, 92, { font: F.displayBold, size: 46, width: 500, lineHeight: 54 });
    text(faqs, "Description", "Clear answers about the current notes, simulations, and educator access.", 56, 226, { font: F.display, size: 18, color: C.muted, width: 420, lineHeight: 31 });
    ...
    const questions = [
      ...
      ["Are the notes useful for JEE Main and JEE Advanced?", ""], ["How does the Simulation Lab help?", ""], ["Can I try the samples without signing in?", ""], ["Who can use educator access?", ""], ["Can I save notes and continue later?", ""]
    ];
    let faqY = 86;
    ...
    publicFooter(root, 3805); return root;
  }
  ...
  function homeFrame(page, index) {
    ...
    text(hero, "Heading / Line 1", "JEE PREP", 56, 105, { font: F.displayBold, size: 62, width: 650, lineHeight: 64 });
    text(hero, "Heading / Line 2", "THAT MAKES", 56, 168, { font: F.displayBold, size: 62, width: 650, lineHeight: 64 });
    text(hero, "Heading / Accent", "HARD IDEAS", 56, 231, { font: F.displayBold, size: 62, color: C.orange, width: 650, lineHeight: 64 });
    text(hero, "Heading / Line 4", "FEEL CLEAR.", 56, 294, { font: F.displayBold, size: 62, width: 650, lineHeight: 64 });
    const underlineA = rect(hero, "Organic underline / 01", 56, 363, 144, 4, C.ember); underlineA.rotation = -2;
    const underlineB = rect(hero, "Organic underline / 02", 196, 359, 145, 4, C.ember); underlineB.rotation = 2;
    const underlineC = rect(hero, "Organic underline / 03", 337, 362, 104, 4, C.ember); underlineC.rotation = -2;
    ellipse(hero, "Organic underline / Dot", 449, 357, 10, 10, C.ember);
    ...
    [["Visual notes", 56], ["Interactive simulations", 208]].forEach(function (value) { text(hero, "Proof / Check / " + value[0], "✓", value[1], 570, { font: F.bold, size: 14, color: C.green }); text(hero, "Proof / " + value[0], value[0], value[1] + 23, 570, { font: F.display, size: 14, color: C.muted }); });
    ...
    text(canvas, "Canvas label", "■  Concept canvas", 20, 17, { font: F.display, size: 12, color: C.muted }); rule(canvas, "Header divider", 0, 48, 624, C.hair);
    const canvasItems = [
      ["Kinematics", "Vertical throw", C.ember, "↑\n●\n↓"],
      ["Waves", "Interference", C.cyan, "∿  ∿"],
      ["Atomic structure", "Orbital probability", "#B48CFF", "⊙"],
      ["Calculus", "Function transform", C.green, "∫  ↗"]
    ];
    canvasItems.forEach(function (value, i) {
      ...
      text(item, "Subject", value[0], 18, 18, { font: F.display, size: 11, color: value[2] });
    ...
    rule(canvas, "Footer divider", 0, 521, 624, C.hair); text(canvas, "Footer copy", "Physics, Chemistry, and Mathematics in motion.", 20, 537, { font: F.display, size: 11, color: C.muted });
    ...
    simNames.forEach(function (value, i) {
      ...
      text(card, "Icon", i === 4 ? "Σ" : i === 2 ? "∿" : "⊙", w - 44, 20, { font: F.mono, size: 17, color: value[2] });
      text(card, "Subject", value[1], 20, 156, { font: F.display, size: 11, color: C.muted }); text(card, "Title", value[0], 20, 184, { font: F.displayBold, size: 19, width: w - 40 });
    ...
    noteNames.forEach(function (value, i) {
      ...
      text(card, "Icon", i < 2 ? "⊗" : i < 4 ? "△" : "Σ", 20, 28, { font: F.mono, size: 18, color: value[3] }); text(card, "Meta", value[1], 20, 104, { font: F.display, size: 10, color: C.soft });
    ...
    const curve = frame(utility, "Marginal utility / Visual", 56, 111, 620, 620, C.surface, C.line); grid(curve, "Visual grid", 0, 0, 620, 620, 46, C.orange, 0.04); text(curve, "Title", "Useful understanding", 28, 28, { font: F.displayBold, size: 23 });
    ...
    const heat = frame(concept, "Concept map / Heat map", 590, 92, 778, 580, C.bg, C.line); const columns = ["Concept", "Foundation", "Connect", "Apply", "Revisit"]; columns.forEach(function (label, i) { text(heat, "Column / " + label, label, i === 0 ? 22 : 180 + (i - 1) * 145, 24, { font: F.display, size: 10, color: C.muted }); });
    ...
    rowsData.forEach(function (value, r) { const y = 64 + r * 81; rule(heat, "Row divider / " + r, 18, y, 742, C.hair); text(heat, "Row label / " + value[0], value[0], 22, y + 27, { font: F.displayBold, size: 14 }); value[1].forEach(function (strength, c) { rect(heat, "Heat / " + value[0] + " / " + c, 176 + c * 145, y + 12, 124, 58, C.orange, C.line, 0, 0.08 + strength * 0.78); text(heat, "Heat value / " + value[0] + " / " + c, Math.round(strength * 100), 176 + c * 145, y + 33, { font: F.display, size: 10, color: C.paper, width: 124, align: "CENTER" }); }); });
  ...
  function sampleNotesFrame(page, route, index) {
    ...
    const formula = frame(sheet, "Core relationship", 34, 490, 560, 118, "#FFF0E8"); rect(formula, "Accent", 0, 0, 4, 118, C.orange); text(formula, "Label", "Core relationship", 20, 20, { font: F.displayBold, size: 11, color: "#8A2F0A" }); text(formula, "Equation", "v² = u² + 2as", 20, 54, { font: F.displayBold, size: 28, color: ink });
    text(sheet, "Retain heading", "Three ideas to retain", 636, 160, { font: F.displayBold, size: 20, color: ink }); ["Acceleration stays downward", "Velocity becomes zero only at the apex", "Equal levels have equal speed magnitude"].forEach(function (label, i) { rect(sheet, "Index block / " + i, 636, 214 + i * 92, 28, 28, ink); text(sheet, "Index / " + i, String(i + 1), 636, 222 + i * 92, { font: F.displayBold, size: 9, color: C.paper, width: 28, align: "CENTER" }); text(sheet, "Idea / " + i, label, 680, 215 + i * 92, { font: F.body, size: 13, color: "#4A454D", width: 265, lineHeight: 21 }); }); rule(sheet, "Aside divider", 636, 500, 322, "#D6D1C8"); text(sheet, "Format label", "Preview format", 636, 530, { font: F.displayBold, size: 11, color: "#8A2F0A" }); text(sheet, "Format copy", "Every note can add its own visual, formula, and retention list without changing the layout.", 636, 561, { font: F.body, size: 13, color: "#4A454D", width: 300, lineHeight: 21 });
  ...
  const routes = [
    spec("Home", "/", "public", [
      ...
      section("Frequently asked questions", "faq", 660, ["What does Orange Nelumbo offer today?", "Does it cover JEE Main and Advanced?", "How does the Simulation Lab help?", "Can I try the samples without signing in?", "Who can use educator access?", "Can I save notes?"], "Clear answers about notes, labs, and educator access.", { alt: true })
    ]),
    spec("About", "/about", "public", [section("Difficult concepts should be easier to see.", "hero", 421, [], "Orange Nelumbo is building a focused JEE learning library around visual notes and interactive simulations."), section("We are reducing the distance between a formula and its meaning.", "cards", 656, ["Make it visible", "Keep it focused", "Let students test it"], ""), section("Start with one concept. See if it clicks.", "hero", 361, [], "The current demo includes sample visual notes and simulations across Physics, Chemistry, and Mathematics.", { alt: true, button: "Explore the library" })]),
    spec("Pricing", "/pricing", "public", [section("Three plans. One visual way to learn.", "hero", 390, [], "Choose visual notes, add interactive simulations, or unlock the complete guided library."), section("Choose your library", "cards", 680, [{ title: "Notes · ₹4,999/year", body: "Complete visual notes, protected reader, bookmarks, and reading progress." }, { title: "Notes + Simulations · ₹6,999/year", body: "Everything in Notes plus the complete interactive simulation library." }, { title: "Complete Library · ₹9,999/year", body: "The complete notes and simulations collection, organised into guided visual sequences." }], "", { columns: 3, cardHeight: 420 })]),
    ...
    spec("Help", "/help", "public", [section("How can we help?", "hero", 278, [], "Quick answers about the learning library and your account."), section("Help topics", "cards", 650, ["Visual notes", "Simulations", "Profile and settings"], "", { columns: 3, cardHeight: 210 }), section("Still need help?", "hero", 201, [], "Tell us which note, simulation, or account screen is causing trouble.", { alt: true, button: "Contact us" })]),
  ...
  function coverFrame(page) {
    ...
    const groups = [
      ...
      { title: "Generation notes", values: ["Desktop canvas: 1440 px", "Hanken Grotesk: primary", "DM Sans: learning and fallback", "Named section frames", "Editable cards, rows, forms and text", "Dynamic routes use [param] templates"] }
    ];
